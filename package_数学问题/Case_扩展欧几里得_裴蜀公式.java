...
/**
 * 扩展欧几里得
 * gcd(a,b) return b==0?a 我们观察到：欧几里德算法停止的状态是：
 * a'= gcd ， b' = 0 ，(a',b'是递归最后一层时参数的值)那么，这是否能给我们求解 x y 提供一种思路呢？ a'x + b'y = gcd 此时x=1,y为任意数
 * 因为，这时候，只要 a = gcd 的系数是 1 ，那么只要 b 的系数是 0 或者其他值 （无所谓是多少，反正任何数乘以 0 都等于 0 但是a 的系数一定要是 1），
 ...
 *
 * 那么ax≡1 (mod p)即ax-yp=1.把y写成+的形式就是ax+py=1，
 * 为方便理解下面我们把p写成b就是ax+by=1。 就表示x是a的模b乘法逆元，y是b的模a乘法逆元。然后就可以用扩展欧几里得求了。
 *
 ...
 * 比如除以a,那就可以乘以a的逆元替代。 （除一个数等于乘它的倒数，虽然这里的逆元不完全是倒数，但可以这么理解，毕竟乘法逆元就是倒数的扩展）。
 */

public class Case_扩展欧几里得_裴蜀公式 {

    static long x ;
    static long y ;

    public static void main(String []args){
        long n = ext_gcd(97 ,-127 ) ;
    }
